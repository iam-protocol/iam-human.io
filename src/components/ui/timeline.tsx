      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          ...
          >
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold font-mono text-muted">
                {item.title}
              </h3>
              {item.content}
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-cyan via-solana-purple to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
