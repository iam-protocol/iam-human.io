...
- **Voice synthesis fingerprinting.** Jitter and shimmer floors/ceilings, HNR bounds, voicing-ratio bounds, F0 delta variance, spectral flatness variance—measured on every dimension where synthesized speech is statistically distinct from a human larynx.
- **Cross-modal temporal coupling.** Voice F0 and hand acceleration are causally coupled within a 50 ms window in real humans (Pouw 2025). A bot stitching audio onto procedural motion fails the cross-correlation peak.
